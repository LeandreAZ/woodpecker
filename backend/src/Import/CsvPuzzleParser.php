...
final class CsvPuzzleParser
{
    ...
    public function parseUpload(?UploadedFile $file): array
    {
        if (!($file instanceof UploadedFile) || !$file->isValid()) {
            throw new BadRequestHttpException('Choisis un fichier CSV valide.');
        }
        if ($file->getSize() > self::MAX_FILE_SIZE) {
            throw new BadRequestHttpException('Le fichier CSV ne peut pas depasser 25 Mo.');
        }
        if ('csv' !== strtolower((string) $file->getClientOriginalExtension())) {
            throw new BadRequestHttpException('Le fichier doit etre au format CSV.');
        }
        $handle = fopen($file->getPathname(), 'rb');
        if (false === $handle) {
            throw new BadRequestHttpException('Le fichier CSV est illisible.');
        }

        try {
            $header = fgetcsv($handle, escape: '\\');
            if (false === $header) {
                throw new BadRequestHttpException('Le fichier CSV est vide.');
            }

            $headers = array_map(
                static fn(string $value): string => strtolower(trim(ltrim($value, "\xEF\xBB\xBF"))),
                $header,
            );
            if (
                [] !== array_diff(self::REQUIRED_HEADERS, $headers) ||
                [] !== array_diff($headers, [...self::REQUIRED_HEADERS, ...self::OPTIONAL_HEADERS]) ||
                count($headers) !== count(array_unique($headers))
            ) {
                throw new BadRequestHttpException(
                    'En-tete CSV invalide: FEN, Moves et Rating sont obligatoires.',
                );
            }
            ...
            while (false !== ($row = fgetcsv($handle, escape: '\\'))) {
                ...
                try {
                    ...
                    if (
                        isset($fingerprints[$puzzle->fingerprint()]) ||
                        (null !== $puzzle->sourceId && isset($sourceIds[$puzzle->sourceId]))
                    ) {
                        $duplicates[] = [
                            'line' => $lineNumber,
                    ...
                    if (null !== $puzzle->sourceId) {
                        $sourceIds[$puzzle->sourceId] = true;
                    }
    ...
    public function normalizeRecord(array $record): NormalizedPuzzle
    {
        ...
        if ('' === $fen) {
            throw new \InvalidArgumentException('FEN obligatoire absente.');
        }
        $this->validateFenAndMoves($fen, $moves);
        if ([] === $moves || [''] === $moves) {
            throw new \InvalidArgumentException('Solution UCI obligatoire absente.');
        }
        foreach ($moves as $index => $move) {
            if (1 !== preg_match('/^[a-h][1-8][a-h][1-8][qrbn]?$/i', $move)) {
                throw new \InvalidArgumentException(
                    sprintf('Coup UCI invalide a la position %d.', $index + 1),
                );
            }
        }
        if (!ctype_digit($rating) || (int) $rating < 100 || (int) $rating > 4000) {
            throw new \InvalidArgumentException('Rating invalide (entier entre 100 et 4000 attendu).');
        }
        foreach (['ratingdeviation', 'popularity'] as $field) {
            if ('' !== trim((string) ($record[$field] ?? '')) && !is_numeric($record[$field])) {
                throw new \InvalidArgumentException(sprintf('%s doit etre numerique.', $field));
            }
        }
        if (
            '' !== trim((string) ($record['nbplays'] ?? '')) &&
            !ctype_digit(trim((string) $record['nbplays']))
        ) {
            throw new \InvalidArgumentException('NbPlays doit etre un entier positif ou nul.');
        }
        if (
            '' !== trim((string) ($record['gameurl'] ?? '')) &&
            false === filter_var($record['gameurl'], FILTER_VALIDATE_URL)
        ) {
            throw new \InvalidArgumentException('GameUrl est invalide.');
        }
            ...
            array_unique(
                array_filter(
                    array_map(
                        ...
                        preg_split('/[;,\s]+/', (string) ($record['themes'] ?? '')) ?: [],
                    ),
                ),
            ),
        );
        $openingTags = array_values(
            array_unique(
                array_filter(
                    array_map(
                        static fn(string $tag): string => strtolower(trim($tag)),
                        preg_split('/[;,\s]+/', (string) ($record['openingtags'] ?? '')) ?: [],
                    ),
                ),
            ),
        );

        return new NormalizedPuzzle(
            '' === $sourceId ? null : $sourceId,
            $fen,
            array_map('strtolower', $moves),
            (int) $rating,
            $themes,
            $openingTags,
        );
    ...
    private function validateFenAndMoves(string $fen, array $moves): void
    {
        ...
        foreach ($moves as $index => $move) {
            if (
                ...
                throw new \InvalidArgumentException(
                    sprintf('Le coup UCI %d est illegal depuis la position courante.', $index + 1),
                );
            }
        }
    }
