...
import { loadStoredSession, saveStoredSession, type AuthSession } from '../features/auth/services/authStorage';
import TrainingsPanelView from '../features/trainings/pages/panel/TrainingsPanelView';
import { StatePanel } from '../shared/ui/StatePanel';
import { previewSession } from '../features/trainings/mocks/previewData';
import type { SupportedTheme } from '../features/solver/services/chessboardPreferences';
import { getRouteDocumentTitle, useAppRoute } from './routing/appRouter';
...
function App() {
  ...
  const locationPreview = getPreviewSessionFromLocation();
  if (locationPreview && session?.token !== locationPreview.token) {
    setSession(locationPreview);
  }
  ...
  return <AuthPage onAuthenticated={handleAuthenticated} sessionMessage={route.name === 'auth' && authPreviewExpired ? 'Reconnecte-toi pour continuer ton entraînement.' : sessionMessage} />;
}
