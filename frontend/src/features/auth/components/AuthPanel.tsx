...
function AuthPanel({ sessionMessage, onAuthenticated }: AuthPanelProps) {
  ...
  const [sessionNotice, setSessionNotice] = useState({ message: sessionMessage, dismissed: false });
  if (sessionNotice.message !== sessionMessage) {
    setSessionNotice({ message: sessionMessage, dismissed: false });
  }
  function dismissSessionMessage() {
    setSessionNotice({ message: sessionMessage, dismissed: true });
  }
  const [dismissedAuthError, setDismissedAuthError] = useState<Error | null>(null);
  ...
  const visibleSessionMessage = sessionNotice.dismissed ? null : sessionMessage;
  const topErrorMessage = !visibleSessionMessage && authMutation.isError && dismissedAuthError !== authMutation.error ? authMutation.error.message : null;
  ...
  function switchMode(nextMode: AuthMode) {
    ...
    setDismissedAuthError(authMutation.error);
    authMutation.reset();
  }
      ...
      {topErrorMessage ? (
        <div className="auth-panel__alert auth-panel__alert--danger" role="alert">
          ...
          <button
            ...
            className="auth-panel__alert-close"
            type="button"
            onClick={() => setDismissedAuthError(authMutation.error)}
          >
            <CircleX aria-hidden="true" size={18} strokeWidth={1.9} />
          </button>
        </div>
