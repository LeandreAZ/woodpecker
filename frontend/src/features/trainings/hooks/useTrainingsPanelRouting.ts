...
import { normalizeRoute, routesEqual } from '../../../app/routing/appRouter';
import { flushSolverNavigationSnapshot } from '../../solver/services/solverPersistence';
...
function useTrainingsPanelRouting({
  ...
  useLayoutEffect(() => {
    ...
    routeTransitionRef.current = routeDidChange;
  ...
  useEffect(() => {
    ...
    if (routeTransitionRef.current && activeView !== targetView) return;

    if (!routesEqual(route, desiredRoute)) {
      onNavigate(desiredRoute, { replace: true });
    }
  }, [activeView, targetView, desiredRoute, onNavigate, route, selectedTraining, trainings, trainingsArePending]);
