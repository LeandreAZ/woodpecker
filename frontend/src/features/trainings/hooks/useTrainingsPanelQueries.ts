import { useUserSettingsOverviewQuery } from '../../settings/hooks/useUserSettingsOverviewQuery';
import { useHistoryOverviewQuery } from '../../history/hooks/useHistoryOverviewQuery';
import { useStatsOverviewQuery } from '../../statistics/hooks/useStatsOverviewQuery';
...
import {
  ...
  getPreviewTrainings,
  isPreviewSession,
} from '../mocks/previewData';
import { apiPathFromIri, buildCycleStats } from '../utils/training.utils';
...
import type { TrainingAttemptHistory, TrainingCycleHistory } from '../../history/types/history.types';


import type { useTrainingsPanelUiState } from './useTrainingsPanelUiState';
import { fetchAllCollection } from '../utils/training.utils';
...
export function useTrainingsPanelQueries(session: AuthSession, uiState: UiState) {
  ...
  const statsOverviewQuery = useStatsOverviewQuery(session, uiState, previewMode);

  const historyOverviewQuery = useHistoryOverviewQuery(session, uiState, previewMode);
  ...
  const userSettingsOverviewQuery = useUserSettingsOverviewQuery(session, uiState, previewMode);
