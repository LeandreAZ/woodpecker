import type { PuzzleCompletionResult, PuzzleSolverSnapshot, Feedback } from '../types/solver.types.ts';
export type { PuzzleCompletionResult, PuzzleSolverSnapshot } from '../types/solver.types.ts';
...
} from '../services/chessboardPreferences';
import '../styles/puzzle-solver.css';
...
export function PuzzleSolver(props: PuzzleSolverProps) {
  const identity = JSON.stringify([props.fen?.trim() || undefined, props.initialEvaluationFailed ?? false, props.solution.map((move) => move.trim()).filter(Boolean)]);
  return <PuzzleSolverBoard key={identity} {...props} />;
}
